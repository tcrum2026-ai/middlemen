...
export default async function BusinessRequestDetailPage({
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <Link href="/dashboard/business/requests" className="text-sm text-stone-600 hover:text-stone-900">
      ...
      <div className="mt-8 rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
        {myOffer ? (
          <div>
            <h2 className="font-semibold text-stone-900">Your offer</h2>
            ...
            <p className="mt-2 text-sm text-stone-600">{myOffer.description}</p>
          ...
          <>
            <h2 className="font-semibold text-stone-900">Submit your offer</h2>
            <div className="mt-4">
              <NewOfferForm requestId={request.id} />
            </div>
          </>
        ) : (
          <p className="text-stone-500">This request is no longer accepting offers.</p>
        )}
      </div>
    </div>
  );
