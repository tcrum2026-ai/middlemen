...
export default async function BusinessDealPage({
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <Link href="/dashboard/business" className="text-sm text-indigo-600 hover:text-indigo-700">
        ← Back to dashboard
      </Link>

      <div className="mt-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">Deal with {deal.customer.name}</h1>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="mt-6 space-y-2 text-sm">
          ...
          <div className="flex justify-between border-t border-slate-200 pt-2">
            <dt className="text-slate-700">You receive</dt>
        ...
        {deal.status === "PENDING_PAYMENT" && (
          <p className="mt-6 text-sm text-slate-500">Waiting for the customer to pay.</p>
        )}
        {deal.status === "PAID" && (
