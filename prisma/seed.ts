...
async function main() {
  ...
  const businessSeeds = [
    ...
    {
      email: "handyheroes@dealbridge.dev",
      name: "Sam Owner",
    ...
    {
      email: "webwrights@dealbridge.dev",
      name: "Lee Owner",
    ...
    {
      email: "growthlane@dealbridge.dev",
      name: "Nina Owner",
  ...
  const businesses = await Promise.all(
    businessSeeds.map(async (b) => {
      const user = await prisma.user.upsert({
        where: { email: b.email },
        update: {},
        create: { email: b.email, name: b.name, passwordHash: password, role: "BUSINESS" },
      });
  ...
  const byCategory = (category: string) => businesses.filter((b) => b.category === category);
  ...
  if (existingRequests === 0) {
    ...
    for (const [biz, price, days, desc] of [
      [byCategory("Home Services")[0], 2400, 5, "Includes pressure wash, premium Sherwin-Williams paint, 2-year warranty, and a 5-person crew."],
      [byCategory("Home Services")[1], 1800, 8, "Budget-friendly option with standard paint, same great prep work, slightly longer timeline."],
    ] as const) {
    ...
    for (const [biz, price, days, desc] of [
      [byCategory("Web & Software")[0], 4200, 14, "Custom Next.js site with Stripe-powered booking, admin dashboard, and 3 months of free support."],
      [byCategory("Web & Software")[1], 2200, 21, "Template-based site with booking plugin, lower cost but longer delivery and limited customization."],
    ] as const) {
      await prisma.offer.create({
        data: {
          requestId: req2.id,
  ...
  console.log(`Businesses: ${businessSeeds.map((b) => b.email).join(", ")}`);
}
